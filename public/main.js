...
socket.on('newUser', function (userName) { // Думаю тут понятно уже =)
    ...
    $('#' + textareaID).val($('#' + textareaID).val() + userName + ' connected!\n'); // Это событие было отправлено всем кроме только подключенного, по этому мы пишем другим юзерам в поле что 'подключен новый юзер' с его ником
});

$(document).on('click', '#sendMsg', function () { // Прослушка кнопки на клик
    var message = [];
    message[0] = $('input').val(); // Все что в поле для ввода записываем в переменную
    message[1] = textareaID;
